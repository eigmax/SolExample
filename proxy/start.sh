...
curl -k -vvvv -s -X POST  -H "Content-type:application/json" --data '{"jsonrpc":"2.0","method":"eth_gasPrice","id":1}'  https://localhost:8443/eth/ | jq .
curl -k -vvvv -s -X POST  -H "Content-type:application/json" --data '{"jsonrpc":"2.0","method":"eth_gasPrice","id":1}'  https://localhost:8443/eig/ | jq .
